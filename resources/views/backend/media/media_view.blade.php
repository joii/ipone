    ...
    <div class="container-full">
		...
		<div class="content-header">
			<div class="d-flex align-items-center">
                ...
                <div class="float-rught"><a href="{{ route('media.add')}}" class="btn btn-outline btn-dark mb-5">Add New</a></div>
			</div>
		</div>	  
            ...
            <div class="row">
                ...
                <div class="box">
					<div class="box-header">						
						<h4 class="box-title">All Media</h4>
                            ...
                            <table id="complex_header" class="table table-striped table-bordered display dataTable" style="width: 100%;" role="grid" aria-describedby="complex_header_info">
								...
								<tbody>	
								    ...
								    <tr role="row" class="odd">
										<td>
										<a href="{{ route('media.edit',$item->id) }}" class="btn btn-info" title="Edit Data"><i class="fa fa-pencil"></i> </a>
 <a href="{{ route('media.delete',$item->id) }}" class="btn btn-danger" title="Delete Data" id="delete">
 	<i class="fa fa-trash"></i></a>
										</td>
										<td>
                                            {{$item->brand_id==''?'':$item->brand_name_en.'/'}}
                                            {{$item->category_id==''?'':$item->category_name_en.'/'}}
                                            {{$item->subcategory_id==''?'':$item->subcategory_name_en.'/'}}
                                            {{$item->product_id==''?'':$item->product_name_en}}

                                        </td>
										<td>{{$item->media_title_th}}<br>{{$item->media_title_en}}<br>{{$item->media_title_ch}}</td>
