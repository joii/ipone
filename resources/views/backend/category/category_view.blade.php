...
@section('admin')
    ...
    <div class="container-full">
		...
		<div class="content-header">
			<div class="d-flex align-items-center">
                ...
                <div class="float-rught"><a href="{{ route('category.add')}}" class="btn btn-outline btn-dark mb-5">Add New</a></div>
			</div>
		</div>	  
 ...
 <a href="{{ route('category.delete',$category->id) }}" class="btn btn-danger" title="Delete Data" id="delete">
 	<i class="fa fa-trash"></i></a>
										...
										<td>{{$category->brand_name_en}}</td>
										<td class="sorting_1"><img src="{{ asset($category->category_image) }}" style="width: 80px;"> </td>
										<td>{{$category->category_name_th}}</td>
										<td>{{$category->category_name_en}}</td>
										<td>{{$category->category_name_ch}}</td>
