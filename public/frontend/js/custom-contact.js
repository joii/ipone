...
$(document).ready(function() {
    // var arrSelectListTH = ["", "ติดต่อธุรกิจต่างประเทศ", "สอบถามข้อมูลผลิตภัณฑ์", "ติดต่อตัวแทนจำหน่าย", "สอบถามข้อมูลทั่วไป", "แนะนำติชม"];
    // var arrSelectListEN = ["", "Contact International Busines", "I.P One Product Information", "Apply for Reseller", "General Information", "Suggestion"];
    // var select = document.getElementById('subject');
    // for (var i = 0; i < 6; i++) {
    //     if (i == 0) {
    //         select.options[select.options.length] = new Option(arrSelectListTH[i], '', true);
    ...
    $(".select.dropdown").each(function() {
        ...
        $selectItem.find(".dropdown-menu li").click(function(i) {
            ...
            if ($(this).index() == 0) {
                $(this).parent().parent().find('a[data-bs-toggle]').html(selText).removeClass('selected');
            } else {
                $(this).parent().parent().find('a[data-bs-toggle]').html(selText).addClass('selected');
            }
